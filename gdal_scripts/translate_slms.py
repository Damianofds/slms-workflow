...
class translate_slms(GdalAlgorithm):
    ...
    def commandLineName(self):
        return "gdalogr:slms-translate"

    def defineCharacteristics(self):
        self.name, self.i18n_name = self.trAlgorithm('SLMS - GeoTIFF retiling and compression')
        ...
        self.addParameter(ParameterSelection(self.RTYPE, self.tr('Output raster type'), self.TYPE, 5))
        self.addParameter(ParameterSelection(self.COMPRESS, self.tr('GeoTIFF options. Compression type:'), self.COMPRESSTYPE, 0))
    ...
    def getConsoleCommands(self):
        out = self.getOutputValue(translate_slms.OUTPUT)
        outsize = unicode(100)
        outsizePerc = False
        ...
        if extra is not None:
            extra = unicode(extra)
        jpegcompression = unicode(75) #unicode(self.getParameterValue(self.JPEGCOMPRESSION))
        predictor = unicode(1) #unicode(self.getParameterValue(self.PREDICTOR))
        zlevel = unicode(6) #unicode(self.getParameterValue(self.ZLEVEL))
